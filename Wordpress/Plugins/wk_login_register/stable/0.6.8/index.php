 ...
 */
function wk_login($language, $already_loggedin_message = '1', $redirect_success = '', $redirect_fail = ''){
 ...
 * 
 * @example wk_register('gr', 'author', 1, '', '/register');
 * 
 * @return 
 * @param string $language
 ...
 * @param string $redirect_fail[optional]
 */
function wk_register($language, $role, $email_notification = 0, $redirect_success = '', $redirect_fail = ''){ ?>

	<?php include_once 'languages/'.$language.'.php'; ?>
	...
	<form id="registerform" action="<?php echo get_option('home'); ?>/wp-content/plugins/wk_login_register/register.php" method="post">
		...
		<input name="submit" type="submit" id="submit" tabindex="4" value="<?php echo $register_txt; ?>" />
	</form>
<?php
}
