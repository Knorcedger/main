		...
		if (($_FILES["file"]["size"] < $size * 1024)) {
			if ($_FILES["file"]["error"] > 0) {
				...
				if ($wh[0] > $width || $wh[1] > $height) {
					...
					if (file_exists("../../uploads/" . $name . '/' . $_FILES["file"]["name"])) {
...
//add mete, or update it if already exists
////////////////////-------------------------------------------------------------------FIX THIS
//add_post_meta($next_post_id, $name, $value, true) or update_post_meta($next_post_id, $name, $value);
