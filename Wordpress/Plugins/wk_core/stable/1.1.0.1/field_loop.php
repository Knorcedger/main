...
foreach ($fields as $val) {
	...
	if ($type == 'textfield') {
		...
		if($object_type == 'post'){
			...
			$temp_photo_path = get_usermeta($object_id, $name);
		}
	...
	<?php
		...
		$search_field = $temp[7];
		$limit = $temp[8];
		$match_contains = $temp[9];
		//$query = "SELECT ID, post_title FROM wp_posts WHERE post_author = 1";
		include_once 'types/autocomplete/autocomplete.php';
		wk_core_autocomplete($display, $name, $translation, $default, $size, $query, $search_field, $limit, $match_contains, $object_id, $object_type);
	}elseif ($type == 'richtexteditor') {
		//save remaining vars
		$media_buttons = $temp[5];
		include_once 'types/richtexteditor.php';
		wk_core_richtexteditor($display, $name, $translation, $default, $media_buttons, $object_id, $object_type);
	}elseif ($type == 'custom_taxonomy') {
		//save remaining vars
		$hierarchical = $temp[5];
		include_once 'types/custom_taxonomy.php';
		wk_core_custom_taxonomy($display, $name, $translation, $default, $hierarchical, $object_id, $object_type);
	}
}
?>
