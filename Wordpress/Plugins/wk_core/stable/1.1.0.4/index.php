...
<?php
/**
 * Submits a post (Default fields post_title, post_content, post_category, tags_input, post_type)
 * 
 * PHOTO_OVER, the params are -> level|type|name|translation|default|Button Upload text~Edit text|Greybox_width~Greybox_height|width|height|size|thumb~thumbwidth~thumbheight~cropratio -> 1|photo_over|avatar|Photo||Upload~Edit|600~400|1000|1000|300|1~80~80~1:1
 * PHOTO_OVER without a thumbnail -> 1|photo_over|avatar|Photo||1000|1000|300|0
 ...
 * DROPDOWN, the params are -> level|type|name|translation|default|var_name~var_value~var_name~var_value -> 1|dropdown|custom_dd|Είδος|2|Μακρύ~long~Μεσαίο~middle~Κοντό~short
 * CHECKBOX, the params are -> level|type|name|translation|default|var_name~var_value~var_name~var_value -> 1|checkbox|mycheckbox|Είδος|1~0~1|Μακρύ~long~Μεσαίο~middle~Κοντό~short
 * AUTOCOMPLETE, the params are -> level|type|name|translation|default|size|query|search_field|limit|match_contains -> 1|autocomplete|ac_test|Autocomplete||30|SELECT ID, post_title FROM wp_posts WHERE post_author = 1|post_title|10|1
 * AUTOCOMPLETE for content_type -> 3|autocomplete|review_company|Company||30||post_title~company|10|1
 * RICHTEXTEDITOR, the params are -> level|type|name|translation|default|media_buttons -> 3|richtexteditor|description|Description||0
 * CUSTOM_TAXONOMY, the params are -> level|type|name|translation|default|hierarchical -> 3|custom_taxonomy|like_tags|Χώρες||0
 *
 * @example wk_submit('gr', 0, array('1|textfield|post_title|Title|default value|30', '0|textfield|fire|Φωτιά|Παντού|30', '1|textarea|post_content|Κείμενο|default value|30|5', '1|date|delivery|Παράδοση|3~12~2009', '0|time|delivery_time|Παράδοση|7~30|30', '1|dropdown|custom_dd|Είδος|2|Μακρύ~long~Μεσαίο~middle~Κοντό~short', '0|checkbox|mycheckbox|Είδος|1~0~1|Μακρύ~long~Μεσαίο~middle~Κοντό~short', '1|photo_over|avatar|Photo||Upload~Edit|300~100|1000|1000|300|1~80~80~1:1'));
 * 
 ...
 * - The RICHTEXTEDITOR displays the tinymce editor with or without upload buttons
 * - The CUSTOM_TAXONOMY displays a custom taxonomy as included in wp 2.8
 * 
 * @return 
...
function wk_submit($language, $object_id, $fields){
	...
	<form class="submitform" action="/wp-content/plugins/wk_core/submit.php" method="post">
		...
		<input type="submit" value="<?php echo $txts['submit']; ?>" />
	</form>
