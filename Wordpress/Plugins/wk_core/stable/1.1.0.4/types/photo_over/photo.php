...
<html>
	<head>
		...
		<link rel="stylesheet" href="/wp-content/plugins/wk_core/types/photo_over/style.css" type="text/css" media="screen" />
	</head>
	<body>
...
if($status == "photo_upload_success" || $temp_photo_path != ''){
	...
	$delete_url = $base . 'types/photo_over/photo_delete.php?object_id=' . $object_id . '&object_type=' . $object_type . '&name=' . $name . '&photo_path=' . $photo_path;
...
<?php
	...
	if($status == 'photo_deleted'){
		//display the form
		include_once $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/wk_core/types/photo_over/photo_form.php';
	?>
		<!--script type="text/javascript">
			photo_path = "";
