<script type="text/javascript">
	jQuery(document).ready(function($) {
		...
		function find_seats(date_id){
			$.post('/wp-content/plugins/wk_content_types/custom/find_seats.php', {object_id: '<?php echo $_GET["post"]; ?>', date_id: date_id},
				function(data){
					//alert("f="+data.seats);
					//if(data.seats != ''){
						//alert(data.seats);
		...
		function main_date_loop(the_year){
			...
			for(i = date_start_month; i <= date_end_month; i++){
				...
				for(j = jstart; j <= days_num; j++){
					...
					var real_month = i+1;
					if(monday == 1 && l == 1){
						var seats = find_seats(dates_num);
			...
			$('<input type="hidden" size="3" value="'+monday+'" id="date_monday" name="dates_monday"></input>').appendTo(".all-dates");
			$('<input type="hidden" size="3" value="'+tuesday+'" id="date_tuesday" name="dates_tuesday"></input>').appendTo(".all-dates");
			$('<input type="hidden" size="3" value="'+wednesday+'" id="date_wednesday" name="dates_wednesday"></input>').appendTo(".all-dates");
			$('<input type="hidden" size="3" value="'+thursday+'" id="date_thursday" name="dates_thursday"></input>').appendTo(".all-dates");
			$('<input type="hidden" size="3" value="'+friday+'" id="date_monday" name="dates_friday"></input>').appendTo(".all-dates");
			$('<input type="hidden" size="3" value="'+saturday+'" id="date_saturday" name="dates_saturday"></input>').appendTo(".all-dates");
			$('<input type="hidden" size="3" value="'+sunday+'" id="date_sunday" name="dates_sunday"></input>').appendTo(".all-dates");
		}
