	...
	if($object_type == 'post'){
		...
		foreach ($vars as $key => $val) {
			if($key == 'post_title'){
				...
				$mypost['post_type'] = $val;
			}elseif($key == 'post_parent'){
				$mypost['post_parent'] = $val;
			}
		}
		...
		if($object_id == '0'){
			...
			if($result != 0){
				...
				foreach ($vars as $key => $val) {
					if($key != 'post_title' && $key != 'post_content' && $key != 'post_category' && $key != 'tags_input' && $key != 'user_id' && $key != 'object_id' && $key != 'object_type' && $key != 'post_type' && $key != 'post_parent'){
		...
		foreach ($vars as $key => $val) {
			...
			if($key != 'user_id' && $key != 'object_id' && $key != 'object_type'){
